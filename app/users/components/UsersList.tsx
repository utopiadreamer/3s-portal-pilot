...
interface GridProps {
  ...
  onReload?: () => void;
  onGridBind?: (usersNum: UserDto[] | undefined) => void;
  onGridChanged?: (filteredColumns: FilteredColumn[]) => void;
  onActionClick?: (action: string, item: UserDto) => void;
  loadData?: (
    pn: number,
    ps: number,
    filterCols?: FilteredColumn[],
    sortedCol?: SortedColumnInfo,
  ) => void;
}

const UsersList: FC<GridProps> = (props: GridProps) => {
  ...
  const gridColumns = [
    ...
    },
    {
      ...
      isResizable: true,
      onRender: ({ id }: UserDto) => {
        return <div className="issueLinkDetails">{id}</div>;
      },
    },
    {
      key: 'sentBy',
      ...
      isRowHeader: true,
      isResizable: true,
      onRender: ({ firstname, lastname, id }: UserDto) => {
        return (
          <Link
  ...
  const onNbItemPerPageChanged = (nbItemPerPage: number) => {
    ...
    setCurrentPage(1);
    if (loadData) loadData(1, nbItemPerPage, filters);
    return true;
  };
  const onChanged = (
    pageIndex: number,
  ...
  const performLoading = () => {
    if (loadData) loadData(currentPage, pageSize, filters);
  };
  ...
  }, [currentLanguage?.lang, currentLanguageProps, performLoading]);
  ...
  return (
    <DetailsList
      ...
      enableShimmer={!users || enableShimmer}
